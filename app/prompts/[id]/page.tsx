...
export default async function PromptDetailPage({
  ...
  return (
    <div className="mx-auto max-w-[680px] px-6 py-9">
      ...
      <div className="my-3.5 flex items-start justify-between gap-3">
        <h1 className="text-2xl font-extrabold text-[#170F2E]">{prompt.title}</h1>
        {user?.id === prompt.user_id && (
          <a
            href={`/prompts/${prompt.id}/edit`}
            className="shrink-0 rounded-lg border px-3 py-1.5 text-[12px] font-bold text-[#4C3FE0]"
      ...
      <PromptBody text={prompt.prompt_body} isLoggedIn={!!user} promptId={prompt.id} />
      ...
      {!user ? (
        <p className="mb-8 text-[11.5px] text-[#8B84A3]">
          プロンプトの全文表示・コピー・お気に入りはGoogleログイン済みのユーザーのみご利用いただけます。
        </p>
      ) : (
        <div className="mb-8" />
      )}
      ...
      <div className="mb-4 flex flex-col gap-3">
        {comments.map((c: any) => (
          <div key={c.id} className="rounded-xl border p-3.5" style={{ borderColor: "rgba(23,15,46,0.10)" }}>
            <div className="mb-1 text-[12.5px] font-bold">
              {c.author?.name ?? "ユーザー"}
              {c.rating ? <span className="ml-2 text-[#F7C948]">{"★".repeat(c.rating)}</span> : null}
        ...
        {comments.length === 0 ? (
          <div className="text-[13px] text-[#8B84A3]">まだコメントはありません。</div>
        ) : null}
      </div>
